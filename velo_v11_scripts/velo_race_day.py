...
def main():
    ...
    for race_id, race_df in df.groupby('race_id'):
        ...
        # Get race cluster
        race_cluster = clustering.predict_race_cluster(race_df.iloc[0])
        ...
        # Get SQPE predictions
        sqpe_outputs = sqpe.predict_for_race(
            race_sample, race_cluster, horse_clusters,
            seq_win_proba, seq_place_proba, intent_scalar
        )
        
        # Get decisions
        decision_model = RaceDecisionModel(sqpe.cfg)
        decisions = decision_model.decide_race(sqpe_outputs, race_cluster)
